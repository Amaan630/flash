...
half3 geomNormalWS, inout half finalSnowMask, float4 vertexColor, out half sssMask, out half highlightBrightness, float2 uv = 0, half dither = 0)
{
    ...
    //calculate UV
	float4 relativeSurfPos = mul(_depthCamMatrix, float4(positionWS, 1.0));
...
*/
    ...
    #ifdef _TRACES_ON
        relativeSurfPos = mul(_SRS_TraceSurfCamMatrix, float4(positionWS, 1));
...
/*
        ...
        #ifdef _STOCHASTIC_ON
            float snowHeight0 = StochasticTex2DLod(_CoverageTex0, sampler_SRS_depth, covTex0_uv, lod).z; //use downsampled height map to prevent high frequency noise
            #if defined (_THREE_TEX_MODE)
                float snowHeight1 = StochasticTex2DLod(_CoverageTex1, sampler_SRS_depth, covTex1_uv, lod).z; //use downsampled height map to prevent high frequency noise
                float snowHeight2 = StochasticTex2DLod(_CoverageTex2, sampler_SRS_depth, covTex2_uv, lod).z; //use downsampled height map to prevent high frequency noise
            #endif
        #else
            float snowHeight0 = SAMPLE_TEXTURE2D_LOD(_CoverageTex0, sampler_SRS_depth, covTex0_uv, lod).z; //use downsampled height map to prevent high frequency noise
            #if defined (_THREE_TEX_MODE)
                float snowHeight1 = SAMPLE_TEXTURE2D_LOD(_CoverageTex1, sampler_SRS_depth, covTex1_uv, lod).z; //use downsampled height map to prevent high frequency noise
                float snowHeight2 = SAMPLE_TEXTURE2D_LOD(_CoverageTex2, sampler_SRS_depth, covTex2_uv, lod).z; //use downsampled height map to prevent high frequency noise
            #endif
        #endif
...
//True triplanar
        ...
        float coverageMasks0X = SAMPLE_TEXTURE2D_LOD(_CoverageTex0, sampler_SRS_depth, uvX, 4).z;
	    float coverageMasks0Y = SAMPLE_TEXTURE2D_LOD(_CoverageTex0, sampler_SRS_depth, uvY, 4).z;
	    float coverageMasks0Z = SAMPLE_TEXTURE2D_LOD(_CoverageTex0, sampler_SRS_depth, uvZ, 4).z;
