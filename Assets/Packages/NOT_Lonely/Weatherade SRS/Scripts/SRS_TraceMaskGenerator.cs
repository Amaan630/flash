namespace NOT_Lonely.Weatherade
{
    ...
    public class SRS_TraceMaskGenerator : MonoBehaviour
    {
        ...
        {
            if (traceObjsRT != null) traceObjsRT.Release();
            if (surfaceDepthRT != null) surfaceDepthRT.Release();
            if (traceMaskRT != null) traceMaskRT.Release();
            if (traceTex != null) traceTex.Release();
            if (finalTraceMask != null) finalTraceMask.Release();
            if (bluredRT != null) bluredRT.Release();

            traceObjsRT = new RenderTexture(maskRes, maskRes, 16, RenderTextureFormat.Depth, RenderTextureReadWrite.Linear);
            surfaceDepthRT = new RenderTexture(maskRes / 2, maskRes / 2, 16, RenderTextureFormat.Depth, RenderTextureReadWrite.Linear);
            traceMaskRT = new RenderTexture(maskRes, maskRes, 0, RenderTextureFormat.R16, RenderTextureReadWrite.Linear);
            traceTex = new RenderTexture(maskRes, maskRes, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
            finalTraceMask = new RenderTexture(maskRes, maskRes, 0, RenderTextureFormat.R16, RenderTextureReadWrite.Linear);
            bluredRT = new RenderTexture(maskRes, maskRes, 0, RenderTextureFormat.R16, RenderTextureReadWrite.Linear);
        ...
        private void SetUvs()
        {
            ...
            traceMaskCalcMtl.SetVector("_UVOffset", uv);
        ...
        public void SimOneStep()
        {
            ...
            SetUvs();
            Shader.SetGlobalMatrix("_SRS_TraceSurfCamMatrix", GL.GetGPUProjectionMatrix(traceSurfCam.projectionMatrix, true) * traceSurfCam.worldToCameraMatrix);
            ...
            blurMtl.SetTexture("_tex", traceMaskRT);
            Graphics.Blit(traceMaskRT, tempRT, blurMtl, 0);
            blurMtl.SetTexture("_tex", tempRT);
            Graphics.Blit(tempRT, bluredRT, blurMtl, 1);
            RenderTexture.ReleaseTemporary(tempRT);

            //Detial Mask
            traceMaskCalcMtl.SetTexture("_SRS_TraceMask", bluredRT);
            ...
            traceMaskCalcMtl.SetTexture("_SRS_TraceMask", finalTraceMask);
            traceMaskCalcMtl.SetTexture("_SRS_TraceMaskBasic", bluredRT);
