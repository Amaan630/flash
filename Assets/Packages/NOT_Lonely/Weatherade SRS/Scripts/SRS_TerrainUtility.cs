namespace NOT_Lonely.Weatherade
{
    ...
    public class SRS_TerrainUtility : MonoBehaviour
    {
        ...
        {
            //set depth to 1 to avoid the "RenderTexture.Create failed: colorFormat & depthStencilFormat cannot both be none." error in URP when DepthNormals pass is used.
            primaryRT = new RenderTexture(basemapResolution.x, basemapResolution.y, 1, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear); 
            ...
            {
                //set depth to 1 to avoid the "RenderTexture.Create failed: colorFormat & depthStencilFormat cannot both be none." error in URP when DepthNormals pass is used.
                secondaryRT = new RenderTexture(basemapResolution.x, basemapResolution.y, 1, RenderTextureFormat.ARGB32);
                packedRT = new RenderTexture(basemapResolution.x, basemapResolution.y, 1, RenderTextureFormat.ARGB32);
